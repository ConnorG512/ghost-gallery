...
Entity::Entity(const std::vector<std::string>& texture_paths,
               ...
               const int x_pos,
               const int y_pos)
    : sprite{texture_paths}, health_component{max_health, current_health}, collision{{x_pos, y_pos}},
      positional_component{x_pos, y_pos}
{
}
...
void Entity::playSound(AudioManager& audio_manager) { audio_manager.playAudio(AudioManager::SoundId::ghost_death); }
