...
UiSplash::UiSplash(const int start_pos_x, const int start_pos_y,
...
void UiSplash::drawUi(const int &player_high_score)
{
    ...
    DrawText(splash_titles.at(0), m_start_position_x, text_y_pos, m_font_size,
             WHITE);
    text_y_pos += m_spacing;

    DrawText(splash_titles.at(1), m_start_position_x, text_y_pos, m_font_size,
             WHITE);
    text_y_pos += m_spacing;

    DrawText(std::format("{}: {}", splash_titles.at(2),
                         std::to_string(player_high_score))
                 .c_str(),
